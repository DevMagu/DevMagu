# 👋 Hola, soy **Magu**

**Desarrollador FullStack apasionado por la creación de experiencias web / Desarrollador Backend 
especializado en Java.**

<div align="center">
  
  🌱 Estoy aprendiendo sobre **Java**. <br>
  🌟 Actualmente enfocado en **Next.JS, React, TypeScript, Tailwind**.
  
  <br>
  💼 ¡Contáctame para trabajos!
  <br><br>
  📫 Email: gutierrezalejandro731@gmail.com
  <br>
  
  <!-- Puedes borrar esta línea si no quieres el avatar flotante -->
  <img src="https://github.com/DevMagu.png?size=100" alt="Foto de perfil" align="right" width="100px">

</div>
<br><br>

---

## 🚀 Lo que hago

Soy un desarrollador especializado en crear soluciones digitales eficientes y escalables.

<details>
<summary>👇 Ver mi stack tecnológico</summary>

- **🧠 Idiomas: Next.JS, JavaScript/TypeScript, HTML, CSS, Java, React, Node.JS**

</details>

---

## 📬 Conectemos

Siento en la necesidad de colaborar en proyectos de código abierto o hablar sobre tecnología.

<div align="center">

| **LinkedIn** | **GitHub** |
| :--- | :--- |
| [[🔗 Link a LinkedIn]](https://www.linkedin.com/in/alejandro-gutierrez-martinez-301335305/) | [[🔗 Link a GitHub]](https://github.com/DevMagu)

</div>

---

<div align="center">

> <em>"La forma en que escribimos el código dice mucho sobre cómo escribimos el código y cómo somos."</em> - Martin Fowler

</div>

---

## 📜 Licencia

La distribución y la copiado de este código es permitido siempre y cuando se mantenga la autoría.
Distribuido bajo la licencia MIT.
Ver la [Licencia](LICENSE) en el mismo directorio.
